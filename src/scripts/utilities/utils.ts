/**
 * Replaces a set of classes to another set of classes and optionally logs the deprecated class usage. Useful for migrating themes from older frameworks.
 * @param classes Array of old and now classes, see the fa4ToFa5 example in this file
 * @param warn Warn about deprecated class usage in the browser console?
 * @param message The type of deprecated classes to indicate in the waring message (if used).
 */
export function replaceClasses(classes: object, warn: boolean = true, message: string = 'deprecated classes'){
    let elements: IClassReplacement[] = [];
    for (const [oldClasses, newClasses] of Object.entries(classes)){
...
/**
 * Replaces all deprecated FontAwesome4 classes to their most lookalike equivalent in FontAwesome5.
 * @param warn Warn about the usage of FontAwesome4 classes ?
 */
export function migrateFa4ToFa5(warn: boolean = true){
    replaceClasses(fa4ToFa5, warn, 'Fontawesome 4 deprecated classes');
}

interface IClassReplacement{
    element: Element,
    oldClasses: string,
    newClasses: string
};

const getSelectorFromClasses = (classes: string): string => {
